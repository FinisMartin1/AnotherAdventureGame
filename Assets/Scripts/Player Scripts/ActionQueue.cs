...
public class ActionQueue : MonoBehaviour
{
        ...
        if (action.IsPreforming == false)
        {
            ...
            switch (action.actionType)
            {
                ...
                case Action.ActionType.Wait:
                    break;
                case Action.ActionType.HavestObjectPlant:
                    PreformHarvestPlantAction(action);
                    break;
                case Action.ActionType.PlantSeed:
                    PreformPlantSeedAction(action);
                    break;
                default:
    ...
    private void PreformHarvestPlantAction(Action action)
    {
        action.ObjectTo.GetComponent<Farmplot>().ProducePlantAndSeed();
    }

    private void PreformPlantSeedAction(Action action)
    {
        action.ObjectTo.GetComponent<Farmplot>().hasSeed = true;
        action.ObjectTo.GetComponent<Properties>().claimedBy = null;
        this.gameObject.GetComponent<Inventory>().invetory.Remove(this.gameObject.GetComponent<Inventory>().invetory.Find(g => g.GetComponent<Properties>().objectId == action.ObjectTo.GetComponent<Farmplot>().GetSeedId(action.ObjectTo.GetComponent<Farmplot>().seed)));
    }
